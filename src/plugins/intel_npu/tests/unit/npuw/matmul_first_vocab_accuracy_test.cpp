...
// A flat weight row would make the MatMul term vanish for any mean-centred activation, so both
// the row and the activation vary - small deviations around the flat values above.
int8_t weight_at(std::size_t k) {
    return static_cast<int8_t>(kWeightValue - static_cast<int8_t>(k % 8));
}

float hidden_at(std::size_t k) {
    return kHiddenValue + static_cast<float>(k % 5) - 2.0f;
}

std::shared_ptr<ov::opset10::Constant> make_weights_constant(const ov::Shape& shape) {
    std::vector<int8_t> values(ov::shape_size(shape));
    for (std::size_t k = 0; k < values.size(); ++k) {
        values[k] = weight_at(k);
    }
...
std::shared_ptr<ov::Model> make_original_lm_head_model() {
    ...
    auto weights = make_weights_constant(ov::Shape{1, kHiddenSize});
...
float evaluate_scalar(const std::shared_ptr<ov::Model>& model) {
    ...
    auto* hidden_data = hidden.data<ov::float16>();
    for (std::size_t k = 0; k < hidden.get_size(); ++k) {
